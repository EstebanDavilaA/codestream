...
- **Cross-tool desync.** Two assistants sharing one `.gsd/` state, each assuming the other had finished. `STATE.json` went stale, cumulative audit logs were overwritten instead of appended (destroying prior milestones' audit trails), spec files were written in the wrong encoding, and two "active" specs coexisted with no way to tell which was real. → Rules 10, 11, 12, 14, 20.
