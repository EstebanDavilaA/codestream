...
1. Spawn the `planner` subagent for the active milestone/phase, reading candidate feature items (`FEAT-xxx`) from `.codestream/FEATURES.md` and bugs/scope gaps (`BUG-xxx`) from `.codestream/BUGS.md`, and setting their status to `IN_PLANNING`.
...
5. An amendment takes effect **only** on re-`SPEC_APPROVED` — rule 1 and rule 5 apply to it exactly as they did to the original. If the amendment is clause-level and the AC matrix is otherwise green, the re-entry is **scoped** (rule 28): `executor` applies only what the amendment authorizes, Layer 1 still runs all four gates in full, and Layer 2 re-audits only the amended clauses plus a regression check on the clauses already labelled `VALIDATED`.
