...
5. Present, filling `<phase status>` from `.codestream/ROADMAP.md`'s "Estimated phases" line for the active milestone against `.codestream/STATE.json`'s `active_phase` (e.g. "phase 1 of an estimated 2 — at least one more phase expected", "phase 2 of an estimated 2 — this is likely the closing phase", or "estimate still unclear, planner will scope it at the next /plan" if the line says TBD):
```
[STATE 4: Steering Checkpoint]
