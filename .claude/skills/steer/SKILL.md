...
4. Spawn the `verifier` subagent to summarize what just completed and write `.slipstream/archive/STEERING_LOG.md` (append a new dated section — never overwrite, rule 12).
5. Present, filling `<phase status>` from `.slipstream/ROADMAP.md`'s "Estimated phases" line for the active milestone against `.slipstream/STATE.json`'s `active_phase` (e.g. "phase 1 of an estimated 2 — at least one more phase expected", "phase 2 of an estimated 2 — this is likely the closing phase", or "estimate still unclear, planner will scope it at the next /plan" if the line says TBD):
```
[STATE 4: Steering Checkpoint]
...
- **Option A (Refine current phase)**:
  1. Record the choice and the user's notes in `.slipstream/archive/STEERING_LOG.md` (append).
...
- **Option B (Proceed Phase)**:
  1. Record the choice in `.slipstream/archive/STEERING_LOG.md` (append).
  2. Archive the current feature spec from `.slipstream/active/` to `.slipstream/archive/specs/` (same filename, no rename needed) and move `.slipstream/active/manual_verification/` contents to `.slipstream/archive/manual_verification/<milestone>_<phase>/`.
  3. Update status of addressed bugs (`BUG-xxx` in `.slipstream/BUGS.md`) and features (`FEAT-xxx` in `.slipstream/FEATURES.md`) to `CLOSED`.
  4. Increment the phase counter in `.slipstream/STATE.json`'s `active_phase`, and add an `"agent"` field to the new `state_history` entry (rule 11).
...
- **Option C (Pivot Roadmap)**:
  1. Record the choice in `.slipstream/archive/STEERING_LOG.md` (append).
  2. Spawn the `roadmapper` subagent to revise `.slipstream/ROADMAP.md` based on what was learned, inspecting `.slipstream/FEATURES.md` and `.slipstream/BUGS.md` for candidate items and updating their status to `SCHEDULED_MILESTONE (Milestone X)`.
...
- **Option D (Complete Milestone)**:
  ...
  3. Update `.slipstream/STATE.json` and `.slipstream/ROADMAP.md` to mark the active milestone complete.
  ...
  5. Advance to the next milestone in `.slipstream/ROADMAP.md` (Phase 1).
