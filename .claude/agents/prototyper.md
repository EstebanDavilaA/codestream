...
- **Never delete, move, or overwrite `CLAUDE.md`, `.claude/`, `.agents/`, or `.codestream/`.** These are the framework, not part of the app you're prototyping. Never run a scaffolding/init command (`npm create vite`, `create-react-app`, template generators, `git clean`, `rm -rf .`, etc.) against the project root, since these tools typically wipe the whole target directory including dotfiles. If you need a fresh scaffold, generate it in a temp directory and copy only the app files into the project.
