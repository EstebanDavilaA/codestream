...
Do not archive on Option A (Refine) — the spec is still active and its **Approved Baseline is never edited**; the refinement comments land as a numbered **amendment** appended to the Amendment Log (rule 25), presented and halted for `SPEC_APPROVED` before any phase transition. If the amendment is clause-level and the AC matrix is otherwise green, the re-entry is scoped (rule 28).
