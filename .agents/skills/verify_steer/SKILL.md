...
1. Confirm the closing spec's **Spec Reconciliation** (rule 23) is present in `.codestream/archive/CRITIC_REPORT.md` for this phase. Rule 23 requires it *before* this archive step, not after — if the section is missing, do not move the spec; send it back to `audit_critic` first.
...
Do not archive on Option A (Refine) — the spec is still active for the current phase. Its **Approved Baseline is never edited**; the user's refinement comments land as a numbered **amendment** appended to the Amendment Log (rule 25), which is presented and halted for `SPEC_APPROVED` before any code execution or phase transition occurs. If the amendment is clause-level and the AC matrix is otherwise green, the re-entry is scoped (rule 28).
