...
2b. Read the spec's **Norms** section and audit it in both directions. (a) Does the implementation actually follow each named norm? Check that the cited precedent still holds and that the new code matches it — do not infer compliance from a green test suite. Where a norm specifies an architectural pattern or port/adapter boundary (referencing `.codestream/templates/DESIGN_PATTERNS.md`), verify the structural separation in code: ensure domain logic does not bypass ports with direct external I/O, ensure declared strategies are not collapsed into inline conditional ladders, and ensure dependencies are injectable without global mutation. (b) Does every norm have a traceable AC row? A norm asserted in a binding section with no AC is an unverifiable claim, and an unverifiable claim is its own finding: the phase's standard was declared but never checked.
...
2b-iii. Where a norm cites `.codestream/templates/UX_HEURISTICS.md`, audit it by exercising the actual behavior — trigger the flow, read the screenshot/walkthrough evidence — not by reading the code and inferring compliance from its shape. Grade any deviation on that file's Critical/Major/Minor/Advisory severity scale in addition to the AC row's YES/NO/PARTIAL: a Critical or Major finding is a Layer 2 FAIL like any other; a Minor or Advisory finding is noted but doesn't block the verdict on its own (route it to `/log` instead).
...
2c-i. **Check every binding literal for a source (rule 26).** A binding clause must not assert a value it does not itself define. For each count, enumeration, permission list, date or arithmetic result appearing as an assertion in Key Behaviors, Resolved Ambiguities, Norms, Safeguards or the Scope Guardrail, ask what derives it — an AC row that computes it, or a clause it is cited from. A literal with no derivation is its own finding: it is hand-maintained prose that no test can check, so it is either already wrong or will go wrong the next time the thing it counts changes. Recompute it yourself where you can. This is not pedantry — a project shipped an AC row asserting a date-window edge as the calendar date `2016-09-25` when `today − 3650 days` was `2016-09-24`, and the row passed for a whole phase because its own test *derived* the boundary and never read the literal.
...
7. **Produce the Spec Reconciliation (rule 23).** Label every clause in the spec's binding sections — Key Behaviors, Resolved Ambiguities, Norms, Safeguards, Scope Guardrail — with exactly one of `VALIDATED`, `CORRECTED`, `DEFECTIVE`, `UNVERIFIABLE`, `SCOPE CREEP`, `SILENT DROP`, citing the AC row that establishes it. A clause with no AC row is `UNVERIFIABLE` even when the code plainly implements it. This section is required on every verdict, PASS or FAIL — on FAIL it is the evidence `/diagnose` needs to tell an implementation bug from a spec error (rule 4), and the input `/plan` needs if the clause itself was wrong. Annotate the spec; never edit its normative text — code does not get to redefine what was asked for.7b. **Resolve the spec through its Amendment Log before labelling anything.** The spec is an Approved Baseline plus an append-only log of numbered amendments (`AM-1`, `AM-2`, …); a reader resolves it by applying the log over the baseline, later entry winning. Label the **resolved** text, and where an amendment is what corrected a clause, cite that amendment alongside the AC row — a `CORRECTED` clause is corrected *by* an amendment, never by a silent in-place edit of the baseline.
7c. **On a scoped re-entry (rule 28), audit only what changed.** When this pass follows an amendment whose AC matrix was otherwise green, re-audit only the amended clauses and the AC rows the amendment added, re-pointed, or retired, plus a regression check that the clauses previously labelled `VALIDATED` still hold. Say explicitly in the report that it is a scoped re-audit and name what was excluded. Scope the review surface — never the verification gates: the four Layer 1 gates are re-run in full regardless, and a green focused suite is not a Layer 1 pass.
