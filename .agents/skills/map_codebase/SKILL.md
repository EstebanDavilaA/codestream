...
5. Hand your findings to `roadmap_slices` so it can produce `.gsd/ROADMAP.md` with Milestone 1 typically being either the stabilization slice (step 2) or "formalize what already works" (step 3) — not a rewrite.
6. Write `.gsd/STATE.json` through the rule 17 read-parse-mutate-serialize-reparse discipline (never a text splice): `current_state: 1` (roadmap exists, planning not begun), `gate_approvals.roadmap_approved: false` (the halt gate flips it), `artifacts.roadmap: ".gsd/ROADMAP.md"`. Append one `state_history` entry for this audit **before** presenting (rule 18), carrying an `"agent"` field (rule 11), then re-read and JSON-parse the file before treating the write as complete.
