...
-1. **First action of a session, always**: if `.codestream/STATE.json` already exists, run the pre-flight integrity check (`.codestream/HARD_RULES.md` rule 10) before anything else — this is a hard block, not a formality.
...
1. Gather: what was expected, what actually happens, and where in the pipeline this was caught (critic report, regression failure, or a bug you noticed while using the app). `.codestream/archive/CRITIC_REPORT.md` is a cumulative log across every milestone — use the milestone/phase id (from the user or `.codestream/STATE.json`) to jump straight to that section rather than reading the whole file.
