...
There is deliberately **no cap on phases per milestone.** A number chosen in advance would be arbitrary — the same feature is one phase in a mature codebase and four in a new one — so sizing is a judgement the roadmap must make and defend, not a limit it passes or fails.
...
7. **Justify every `Estimated phases` value, and treat it as a first guess, not a promise** — the actual split is often only clear once `/plan` scopes the work. State which of the four axes produced the number (or "1 — single capability, no unmet dependency"). `plan_spec` updates this line as its own understanding sharpens; this field exists so `/steer` can tell the user, at each phase's checkpoint, whether more phases are still expected — never leave it blank and never leave it unargued.
