...
- Any gate fails → do not proceed to `/steer`. Route through `/diagnose` before any fix (rule 4) — the cause decides whether it returns to `/execute` or goes up to `/plan`.
