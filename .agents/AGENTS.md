...
**Gemini-specific addendum — not a canonical rule number; it has no counterpart in the shared rules section.** Kept here, outside that section, so that keeping the three directive copies byte-identical cannot destroy it.

- **Precedence over Standard Planning Mode and strict halt enforcement.** CODESTREAM Hard Rules override default IDE planning-mode exceptions (such as skipping plans for "minor follow-ups" or "simple tweaks"), **except for the lightweight-task exception in rule 7**. At any halt gate (`/discover`, `/plan`, `/steer`, `/map`, `/promote`), Gemini MUST NOT invoke file-modification tools (`replace_file_content`, `write_to_file`, `multi_replace_file_content`) on workspace codebase files, and must yield the turn to the user immediately.
- **Rule 14, encoding.** If a file-write tool in this environment has a known encoding quirk on Windows, prefer explicitly specifying UTF-8 on write rather than relying on a platform default.
