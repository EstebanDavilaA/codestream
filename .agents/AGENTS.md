...
When `/steer` (or the `verify_steer` persona) archives a spec into `.slipstream/archive/specs/` at milestone/phase closure, the corresponding copy in `.slipstream/active/` is removed as part of that same action — verified afterward by re-listing `.slipstream/active/`, not assumed to have succeeded. A byte-identical duplicate left behind in `active/` is exactly the ambiguity rule 10 point 3 exists to catch in whichever session opens next.
...
- **Constraints**:
  ...
  - Do not write tests, docs, or `.slipstream/` artifacts.
...
- **Constraints**:
  ...
  - Derives criteria independently from spec in `.slipstream/active/` (including binding resolved ambiguities, fallback retention rules, stateful integration contracts, and scope guardrails).
...
- **Constraints**:
  ...
  - Write `.slipstream/archive/STEERING_LOG.md` and manage archiving of active specs upon milestone/phase closure.
...
- **Goal**: Safely restore codebase and `.slipstream/` state to a verified clean point when implementation breaks features or corrupts project health.
- **Traits**: Methodical, cautious, state-preserving.
- **Constraints**:
  - Always verify baseline test suite after reset.
  - Keep `.slipstream/STATE.json` state history intact with an explicit `RESET` entry.
  - Never discard work without preserving or listing impacted files.

---

## Shared Runtime Directory (`.slipstream/`) Reference
