...
This file is the single source of truth for the framework's non-negotiable rules. Every tool-specific directive file embeds a verbatim copy of this file's `## Hard rules` section, because each directive must be self-contained and auto-loaded by its own tool — a pointer alone is not reliably read. Nothing in that section names a tool: a directive declares its own identity in its own header, and the rules are written tool-agnostically so every copy stays character-for-character identical. **When these rules change, edit this file first, then bring every directive copy into line with it in the same sitting.** If any copy disagrees, this file wins, and the disagreement itself is a bug to fix immediately, not a judgment call to make in the moment. `scripts/check-framework.py` fails CI when a copy drifts, rather than letting the divergence surface in a project.
...
- **The repair loop had no legal route.** Rule 4 sends a spec error to `/plan`; rule 23 sent the correction to the *next* phase's spec; `/steer` Option A revised the active spec in place; and `/plan` itself defined only how to draft a *new* spec, with no amendment mode at all — four different destinations for one event, and the two rules that named a destination pointed at a mode that did not exist. Every project therefore invented its own "surgical amendment" protocol and re-derived it from the last one, paying a full diagnose → amend → re-approve → re-execute round-trip each time (one project: six such amendments, ~49 re-approvals, and four consecutive audit cycles on a single phase). Two further causes compounded it: binding clauses carrying hand-maintained derived literals, which one clause change cascades across (13 coordinated corrections in a single amendment), and a plan-time gate that checked a spec's *structure* but never its *coverage or internal consistency*, so a binding clause with no AC row was discoverable only after a full build. → Rules 25, 26, 27, 28.
